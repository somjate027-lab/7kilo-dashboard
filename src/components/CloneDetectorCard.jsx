  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-end justify-between">
        <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Risk Score</span>
        <span className={`text-2xl font-black ${textColor}`}>{displayed}<span className="text-sm font-semibold text-slate-400">/100</span></span>
      </div>
  ...
  return (
    <div className="w-full max-w-sm rounded-2xl overflow-hidden border border-slate-200 shadow-lg bg-white">
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 px-4 py-3 flex items-center gap-3">
        ...
        <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/10 text-slate-300 font-medium flex-shrink-0">
          AI Analysis
        </span>
      </div>
          ...
          <>
            ...
            {scanData?.top_risk_reason && (
              <div className="flex items-start gap-2 bg-red-50 border border-red-100 rounded-xl px-3 py-2">
                ...
                <p className="text-xs text-red-700 font-medium leading-snug">{scanData.top_risk_reason}</p>
              </div>
            )}
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-xl px-3 py-2.5">
              <span className="text-xs text-slate-500">พบประกาศที่ตรงเงื่อนไข</span>
              <span className="text-sm font-black text-slate-800">
                {totalFound} <span className="text-xs font-semibold text-slate-400">ประกาศ / 5 แพลตฟอร์ม</span>
              </span>
            </div>
            ...
            <div className="relative rounded-xl border border-slate-100 overflow-hidden">
              <div className="px-3 py-2 space-y-2 select-none pointer-events-none">
                ...
                <div className="h-3 bg-slate-200 rounded-full w-2/3 blur-sm" />
                <div className="flex gap-2">
                  <div className="h-8 w-12 bg-slate-200 rounded blur-sm" />
                  <div className="h-8 flex-1 bg-slate-200 rounded blur-sm" />
                </div>
              </div>
              {/* Overlay */}
              <div className="absolute inset-0 bg-white/70 backdrop-blur-[2px] flex flex-col items-center justify-center gap-1">
                <Lock className="w-4 h-4 text-slate-400" />
                <p className="text-[11px] text-slate-500 font-medium">ลิงก์ประกาศ + หลักฐานเต็ม</p>
              </div>
            </div>
            ...
            <button
              onClick={onLoginSave}
              className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 text-white rounded-xl py-3 px-4 text-sm font-bold shadow-lg shadow-blue-500/25 transition-all hover:-translate-y-0.5 active:translate-y-0"
            >
