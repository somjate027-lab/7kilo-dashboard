...
// Tagline สุ่มสลับ — กลิ่น retro 90s + มู/ดวง (เลเยอร์บุคลิก ไม่แตะผล/CTA)
const RETRO_TAGLINES = [
  '🔮 ดวงว่าได้คืน — ผมขุดให้แน่อีกที',
  '🔮 หมอดูชี้ทิศ ผมขุดหลักฐาน — ทำงานคู่กัน',
  '🔮 ปีชงไม่ต้องกลัว — มีหลักฐานช่วยดวง',
];
...
const AGENT_OPTIONS = [
  ...
  { key: 'DOC_FORGE',      label: 'Doc',      icon: FileWarning,color: 'text-amber-600',  bg: 'bg-amber-50' },
  { key: 'SCRAPER',        label: 'Scraper',        icon: Globe2,     color: 'text-teal-600',   bg: 'bg-teal-50' },
];

export default function MainContent({ user, requestLogin, newChatSignal }) {
  const [showTag, setShowTag] = useState(true);
  const [tagline] = useState(() => RETRO_TAGLINES[Math.floor(Math.random() * RETRO_TAGLINES.length)]);
  ...
  // เก็บ messages ล่าสุดไว้ใน ref ให้ resetConversation อ่านได้โดยไม่ stale
  const messagesRef = useRef(messages);
  useEffect(() => { messagesRef.current = messages; }, [messages]);

  // เริ่มสนทนาใหม่ — สรุปคดีก่อนล้าง (ถ้ามีข้อความพอ)
  ...
  // เมื่อ logo / sidebar สั่ง "หน้าหลัก" จากภายนอก → reset (ข้าม mount แรก)
  ...
  // ปุ่ม CTA ชมพู — บังคับเด้งการ์ดสแกน Clone Detector ทันที (ไม่พึ่ง LLM เดา route)
  const startCloneScanDemo = (text) => {
    if (isLoading) return;
    // ถ้า login แล้ว → ใช้ flow ปกติ (เข้า agent จริง)
    if (user) { handleSendMessage(text); return; }

    const now = Date.now();
    const userMessage  = { id: `${now}`,     role: 'user',        content: text, timestamp: new Date() };
    const introMessage = { id: `${now + 1}`, role: 'assistant',   content: 'OK ผมจะตรวจให้ 5 จุดหลักการใช้ AI Clone:\n\n1. รอยตัวถัง/สติ๊กเกอร์เดิม – AI ลบไม่เนียนจะเหลือเงา\n2. แสงเงาไม่สอดคล้อง – สีใหม่แต่เงาล้อ/พื้นยังเป็นสีเดิม\n3. ทะเบียน/เพลท – ตัวอักษรเบลอผิดธรรมชาติ\n4. มุมกล้องซ้ำ – ใช้รูปเดิมแต่หมุน/ครอป\n5. พื้นหลังซ้ำ – AI เปลี่ยนรถแต่ลืมเปลี่ยนเงาตึก\n\nอัปโหลดรูปต้นฉบับ 1 รูป + รูปที่สงสัย 1-3 รูป มาได้เลยครับ เดี๋ยวผมเทียบให้ทันที หรือจะให้ผมตรวจจากชื่อยี่ห้อรถก็พิมพ์มาได้เลย 👇', timestamp: new Date() };
    const scanCardMsg  = { id: `${now + 2}`, role: 'scan-result', scanType: 'CLONE_DETECTOR', scanData: null, timestamp: new Date() };

    setMessages(prev => [...prev, userMessage, introMessage, scanCardMsg]);

    analyzeCloneRisk([...messagesRef.current, userMessage])
      .then(data => {
        setMessages(all => all.map(m => m.id === scanCardMsg.id ? { ...m, scanData: data } : m));
      })
      .catch(() => {});
  };

  // ปุ่ม 3 — บังคับเด้งการ์ด Bot Hunter (เคสตัวอย่างรีบขาย) ทันที
  const startBotHunterDemo = (text) => {
    if (isLoading) return;
    if (user) { handleSendMessage(text); return; }

    const now = Date.now();
    const userMessage  = { id: `${now}`,     role: 'user',      content: text, timestamp: new Date() };
    const introMessage = { id: `${now + 1}`, role: 'assistant', content: 'รับเคสแล้ว — ขอยกตัวอย่างโพสต์รีบขายต้องสงสัยมาวิเคราะห์พฤติกรรมให้ดูก่อน 👇', timestamp: new Date() };
    const scanCardMsg  = { id: `${now + 2}`, role: 'scan-result', scanType: 'BOT_HUNTER', scanData: null, timestamp: new Date() };

    setMessages(prev => [...prev, userMessage, introMessage, scanCardMsg]);
  };

  // ปุ่ม 4 — เด้งการ์ดล่อบอท (interactive) ทันที
  const startBrokerBotDemo = (text) => {
    if (isLoading) return;
    if (user) { handleSendMessage(text); return; }

    const now = Date.now();
    const userMessage  = { id: `${now}`,     role: 'user',      content: text, timestamp: new Date() };
    const introMessage = { id: `${now + 1}`, role: 'assistant', content: 'ลองแกล้งเป็นผู้ซื้อทักคนขายดูก่อน — พิมพ์ถามเรื่องเล่มทะเบียน เลขตัวถัง หรือนัดดูรถ ผมจะจับสัญญาณว่าเป็นบอทเฝ้ารถไหม 👇', timestamp: new Date() };
    const scanCardMsg  = { id: `${now + 2}`, role: 'scan-result', scanType: 'BROKER_BOT', scanData: null, timestamp: new Date() };

    setMessages(prev => [...prev, userMessage, introMessage, scanCardMsg]);
  };

  // ปุ่ม 2 — เด้งการ์ด Takedown teaser (เจอรถถูกโพสต์ขายซ้ำ → ล่อ login)
  const startTakedownDemo = (text) => {
    if (isLoading) return;
    if (user) { handleSendMessage(text); return; }

    const now = Date.now();
    const userMessage  = { id: `${now}`,     role: 'user',      content: text, timestamp: new Date() };
    const introMessage = { id: `${now + 1}`, role: 'assistant', content: 'กำลังตามล่ารูปรถคุณข้ามเว็บมือสอง — ถ้าโจร AI เปลี่ยนสี/ตัดต่อแล้วเอาไปโพสต์ขาย ผมจะหาเจอและเตรียมถอดให้ 👇', timestamp: new Date() };
    const scanCardMsg  = { id: `${now + 2}`, role: 'scan-result', scanType: 'TAKEDOWN', scanData: null, timestamp: new Date() };
      ...
      <div className="flex-1 flex flex-col h-full w-full min-w-0 bg-[#0a0a0a] sm:bg-grid-pattern sm:bg-[#fcfdfe] relative overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b-0 sm:border-b border-white/0 sm:border-slate-150/60 bg-[#0a0a0a]/90 sm:bg-white/80 backdrop-blur-md px-4 md:px-8 py-3 sm:py-4 sticky top-0 z-10 sm:shadow-3xs">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="text-sm font-bold text-white sm:text-slate-800 flex items-center gap-2">
                <span className="hidden sm:inline">@7กิโล๊ะ — นักสืบรถหาย</span>
          ...
          <button
            ...
            className="inline-flex items-center space-x-1.5 px-2.5 sm:px-3.5 py-1.5 sm:py-2 hover:bg-white/[0.06] sm:hover:bg-indigo-50 border border-white/[0.08] sm:border-slate-205 hover:border-white/20 sm:hover:border-indigo-200 rounded-xl text-xs font-bold text-slate-400 sm:text-slate-500 hover:text-white sm:hover:text-indigo-600 transition-all cursor-pointer sm:shadow-3xs disabled:opacity-50"
            disabled={isSummarizing}
          >
            <MessageSquarePlus className="w-4 h-4 sm:w-4 sm:h-4" />
            <span className="hidden sm:inline">{isSummarizing ? 'กำลังสรุปคดี...' : 'เริ่มสนทนาใหม่'}</span>
          </button>
        </div>
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            ...
            {messages.map((msg) => (
              ...
              >
                ...
                {/* Scan Result Card — Takedown teaser */}
                {msg.role === 'scan-result' && msg.scanType === 'TAKEDOWN' && (
                  <div className="w-full max-w-sm ml-0">
                    <TakedownCard
                      onLoginSave={() => requestLogin?.(
                        'CLONE_DETECTOR',
                ...
                {/* Scan Result Card — Broker Bot (interactive) */}
                {msg.role === 'scan-result' && msg.scanType === 'BROKER_BOT' && (
                  <div className="w-full max-w-sm ml-0">
                    <BrokerBotCard
                      onLoginSave={() => requestLogin?.(
                        'BOT_HUNTER',
  ...
  return (
    <div className="flex-1 overflow-y-auto relative bg-[#0a0a0a] sm:bg-grid-pattern sm:bg-[#fcfdfe]">
      ...
      <div className="sm:hidden flex flex-col px-6 pt-6 pb-[180px] bg-[#0a0a0a]">
        ...
        {!user && (
          <div className="mb-4">
            <BetaBanner onRegister={() => requestLogin?.('CLONE_DETECTOR', [])} />
          </div>
        )}
        ...
        <div className="mb-2">
          ...
          {/* Main greeting */}
          <h1 className="text-[17px] font-normal text-white tracking-wide mb-1">
            แก้ชงรถหาย? ทันที
          </h1>
          <p className="text-[13px] text-[#aaa] tracking-wide mb-2">
            {tagline}
          </p>
        </div>
        ...
        <button
          onClick={() => startCloneScanDemo('รถผมถูกขโมย สงสัยโจรใช้ AI Clone ดัดแปลงรูป (เปลี่ยนสี/มุม/ทะเบียน) เอาไปลงขาย — ช่วยค้นโพสต์ขายที่น่าสงสัย + สแกนหาจุดภาพปลอม (Image Forgery) ให้ทีครับ')}
        ...
        <div className="space-y-2 mb-4">
          {[
            { text: '🛡️ ตามล่ารูปรถ + สั่งถอดประกาศขาย', demo: 'TAKEDOWN', prompt: 'รถผมถูกขโมย สงสัยโจรเอารูปไปเปลี่ยนสี/ตัดต่อแล้วโพสต์ขายซ้ำหลายเว็บ — ช่วยตามล่าว่าถูกโพสต์ที่ไหนบ้าง แล้วสั่งถอดประกาศให้หน่อย' },
            { text: '🔥 โพสต์รีบขายผิดปกติ + ยิงแอดหนัก', demo: 'BOT_HUNTER', prompt: 'เจอโพสต์ขายรถน่าสงสัย ราคาถูกกว่าตลาดมาก โพสต์ซ้ำหลายกลุ่ม ใช้คำเร่งด่วน รีบปิดการขาย แถมยิงแอดหนัก — ช่วยวิเคราะห์พฤติกรรมว่าเข้าข่ายรถถูกขโมยไหม' },
            { text: '🕵️ แกล้งเป็นผู้ซื้อ ล่อบอทเฝ้ารถขโมย', demo: 'BROKER_BOT', prompt: 'เจอประกาศขายรถที่คล้ายรถที่ผมหายไป อยากลองทักไปคุยเหมือนผู้ซื้อ เพื่อจับว่าคนขายเป็น AI บอทที่โจรตั้งไว้เฝ้ารถไหม' },
          ].map((s, i) => (
            <button
              key={i}
              onClick={() => s.demo === 'TAKEDOWN' ? startTakedownDemo(s.prompt)
                          : s.demo === 'BOT_HUNTER' ? startBotHunterDemo(s.prompt)
                          : s.demo === 'BROKER_BOT' ? startBrokerBotDemo(s.prompt)
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between max-w-4xl mx-auto mb-6 md:mb-8 bg-transparent gap-3 md:gap-0">
        ...
        <div className="flex items-center space-x-3 md:space-x-4">
          ...
          <div>
            <div className="text-[10px] md:text-xs font-bold text-indigo-500 uppercase tracking-wider">แก้ชงรถหาย</div>
